...
//			System.out.println();
	...
	public void assignment(Courier courier,Line line) //按某种规则分配订单
	{
		...
		for(String key : orderMap.keySet()) //过滤掉不满足条件的商户
		{
			...
			{
				//到达商户的时间早于最晚到的时间
				if(courier.current_time+shops.get(key)<=orderMap.get(key).get(i).last_time)
				{
					if(orderMap.get(key).get(i).pickup_time < earlestTime) //选择最早的商户订单
					{
		...
		else
		{
			...
			for(int i=0;i<orderMap.get(earlestShop).size();i++)
			{
				if(orderMap.get(earlestShop).get(i).order_id == order.order_id)
				{
					...
					{
						courier.current_time = courier.current_time+shops.get(earlestShop)+order.time+order.stay_time;
...
//					System.out.println("快递员   "+order.shop_id+"  "+(courier.current_time-order.time-order.stay_time)+"  "+
//							(courier.current_time-order.time-order.stay_time)+"  "+order.num+"  "+order.order_id);
//					System.out.println("快递员   "+order.spot_id+"  "+(courier.current_time-order.stay_time)+"  "+
...
//				System.out.println("快递员   "+order.shop_id+"  "+order.pickup_time+"  "+order.pickup_time+"  "+
//							order.num+"  "+order.order_id);
//				System.out.println("快递员   "+order.spot_id+"  "+(order.pickup_time+order.time)+"  "+
//						(order.pickup_time+order.time+order.stay_time)+"  -"+order.num+"  "+order.order_id);
			}
		}
