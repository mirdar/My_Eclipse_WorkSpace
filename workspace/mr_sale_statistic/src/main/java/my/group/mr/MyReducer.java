...
//		HashMap<Long,Double> map = new HashMap<Long,Double>();
		...
		int neg = 0;
		double perday = 0;
		for (int i = 10; i <= 443; i++)
		{
			if(i<443 )
			{
				if(flag == 1 && temp[i] == null && temp[i+1] != null){ //平滑中间缺失值
					temp[i] = (temp[i-1]+temp[i+1])/2;
				}
				if(flag == 1 && temp[i] == null && temp[i+1] == null){ //不平滑中间缺失值,直接用0填充
					temp[i] = 0.0;
				}
			}
			else
			{
				if(flag == 1 && temp[i] == null)
				{
					temp[i] = temp[i-1];
				}
			}
		...
		result.set(j,neg); //前面多少天未出现
//		if(neg == 443)
//			result.set(j+1,0);
//		else
//			result.set(j+1,perday/(443-neg)); //出现后每天平均销量
//		result.set(j+2,perday);
		double week = 0;
		double two_week=0;
		for (int i = 10; i <= 443; i++) //统计的是周和，两周和
		{
			week+=temp[i];
			two_week+=temp[i];
			if((i-9)%7 == 0)
...
//				result.set(++j,week/7);
			...
			if((i-9)%14 == 0)
			{
				result.set(++j,two_week);
//				result.set(++j,two_week/14);
				two_week = 0;
			}
		}
